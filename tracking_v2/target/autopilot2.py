...
def _plot_path(commands: List[Union[Straight, Turn]]):
    x, y = [], []
    pts_x, pts_y = [], []
    ...
    for command in commands:
        if isinstance(command, Straight):
            ...
            alpha = np.linspace(command.alpha0, command.alpha1, 20)
...
class AutopilotTarget(Target):
    def __init__(self, commands: List[Union[Straight, Turn]], max_turn_rate_deg: float = 5,
                 mass_kg: float = 1500, thrust_N: float = 2000, drag_coefficient: float = 0.05,
                 noise_intensity: float = 1, integration_steps_count: int = 20,
                 seed: int = None):
        
        self.commands = commands
        
        self.max_turn_rate = max_turn_rate_deg / 180 * np.pi # radians
